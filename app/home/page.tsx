    ...
    >
      <div className="md:w-1/4 min-h-screen p-5 border-r-2 border-r-neutral-800 hidden md:flex flex-col items-center gap-7">
        ...
        <div className="w-full bg-neutral-400 bg-opacity-30 rounded-lg flex flex-col gap-5 px-1 md:px-3 py-2 border-2 border-opacity-10 border-neutral-100">
          ...
          <h1 className="font-bold">
            Designation : <span className="font-medium">{desig}</span>
          </h1>
        </div>
        </div>
        <div className="w-full flex flex-col gap-1">
      ...
      <div className='md:w-3/4 w-full min-h-screen flex flex-col p-5 items-center justify-center'>
            <div className=' p-5 rounded-lg flex flex-col gap-5 items-center'>
                <button className='px-8 py-1 bg-blue-600 rounded text-xl font-semibold flex items-center gap-3 hover:bg-blue-800' onClick={()=>setIsOpen(true)}><PlusIcon className='w-6 h-6'/>Add new</button>
                <button className='px-8 py-1 bg-blue-600 rounded text-xl font-semibold flex items-center gap-3 hover:bg-blue-800' onClick={()=>setIsStepModal(true)}><EnterFullScreenIcon className='w-6 h-6'/>Scan</button>
            </div>
      </div>
      {isOpen && (
          <AddProduct close={handleModalClose}/>
        )}
      {isStepModal && (
        <AddStep close={handleStepModalClose}/>
      )}
    </div>
  );
}
